# Wayfinders — Claude Code Context

This file is the authoritative source of truth for the Wayfinders product. Treat it as
canonical over anything inferred from the current state of the code — the code is early
and incomplete; this document reflects the full intended product.

**For the full database schema (27 tables, field by field), see `DATA-MODEL.md` in this
same folder — it is kept separate from this file to stay under Claude Code's context
size limit. Read it explicitly whenever doing schema or database work.**
